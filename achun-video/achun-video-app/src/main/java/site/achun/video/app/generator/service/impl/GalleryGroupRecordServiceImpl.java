package site.achun.video.app.generator.service.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import site.achun.video.app.generator.domain.GalleryGroupRecord;
import site.achun.video.app.generator.service.GalleryGroupRecordService;
import site.achun.video.app.generator.mapper.GalleryGroupRecordMapper;
import org.springframework.stereotype.Service;

/**
* @author Administrator
* @description 针对表【gallery_group_record】的数据库操作Service实现
* @createDate 2023-05-29 17:17:29
*/
@Service
public class GalleryGroupRecordServiceImpl extends ServiceImpl<GalleryGroupRecordMapper, GalleryGroupRecord>
    implements GalleryGroupRecordService{

}





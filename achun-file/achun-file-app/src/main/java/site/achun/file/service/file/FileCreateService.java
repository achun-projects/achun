...
public class FileCreateService {
    ...
    public InitFileInfoResponse initFileInfoV2(InitFileInfo initFileInfo) {
        ...
        if(existFileInfo != null){
            ...
            if(StrUtil.isEmpty(initFileInfo.getFileCode())){
                String code = MD5.create().digestHex(initFileInfo.getAbsolutePath());
                String unitCodeSubstring = StrUtil.isEmpty(initFileInfo.getUnitCode()) ? "" : initFileInfo.getUnitCode().substring(0, 4);
                fileCode = storage.getStorageCode().substring(0,4) + unitCodeSubstring + code.substring(0,24);
            }else{
                fileCode = initFileInfo.getFileCode();
            }
...
//    }
    ...
    private FileInfo toFileInfo(InitFileInfo initFileInfo){
        ...
        fileInfo.setDirCode(initFileInfo.getDirCode());
